<template>
	<div>
		<div class="card">
            ...
            <div class="card mt-5">
                <div class="card-header">Avalliable Dates this month:</div>
                <div class="card-body">
                    <table class="table table-striped">
                        ...
                        <tbody>
                        <tr v-for="(r,index) in appoinmtents" >
...
<script type="text/javascript">
	...
	export default{
		...
		mounted(){
		    ...
		    // let a = (!date) ? this.date : date;
			let time = moment(this.date).parseZone("Australia/Melbourne");
			this.loading=true
			axios.get('/api/doctors/today').then((response)=>{
				this.doctors = response.data
				this.loading=false
			})
            axios.get('/api/appointems').then((response)=>{
                this.appoinmtents = response.data
			})
		}
	}
</script>
